...
def test_open_captcha_review_clears_stale_error(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        web_app,
        "_captcha_job_row",
        lambda session, job, include_next_review_url=False: {
            "open_url": "https://example.invalid/review"
        },
    )
    monkeypatch.setattr(
        web_app,
...
def test_captcha_review_job_exposes_next_page_url(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda session, job, include_next_review_url=False: {
            ...
            "next_open_url": (
                web_app.build_reviews_url(
                    "https://www.emag.ro/example/pd/ABC123/",
                    offset=200,
                )
                if include_next_review_url
                else None
            ),
        },
    )
...
def test_captcha_brand_job_does_not_request_review_next_url(monkeypatch) -> None:
    class FakeJob:
        def __init__(self) -> None:
            self.id = 314
    ...
    monkeypatch.setattr(
        web_app,
        "_captcha_job_row",
        lambda session, job, include_next_review_url=False: calls.append(
            include_next_review_url
        )
        or {
            "job": job,
            "brand_slug": "samsung",
            "family_name": None,
            "open_url": job.target_url,
            "next_open_url": None,
        },
    )
    ...
    request = Request(
        {
            "type": "http",
            "method": "GET",
            "path": "/captcha/brand/314",
            "headers": [],
            "app": web_app.app,
            "router": web_app.app.router,
        }
    )
    response = web_app.captcha_brand_job(request, 314)
